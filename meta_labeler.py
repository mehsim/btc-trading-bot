...
def train_meta_labeler() -> tuple[bool, str]:
    ...
    try:
        ...
        if len(df_trades) < 15:
            return False, f"Insufficient trade labels ({len(df_trades)} < 15)"
        ...
        log_event("INFO", f"[MetaLabeler] Trained on {len(y)} trades | Historical Win Rate: {y.mean()*100:.1f}%")
        return True, f"MetaLabeler trained on {len(y)} trades"
...
def evaluate_meta_filter(symbol: str, interval: str, direction: str, min_prob: float = 0.40) -> tuple[bool, float]:
    ...
    if "model" not in _META_MODEL_CACHE:
        ...
        if not success or "model" not in _META_MODEL_CACHE:
            return True, 0.50  # Fail-open baseline if meta-model unavailable
