...
# Compute production-aligned MIN_FLOOR threshold
p_star = SL_MULT / (TP_MULT + SL_MULT)  # 1.0 / 3.5 = 0.2857
cost_bps = 16.0  # 0.16% round-trip fees = 16 bps
eval_threshold = round(min(0.52, max(MIN_EVAL_THRESHOLD_FLOOR, p_star + (cost_bps / 1e4) / (TP_MULT + SL_MULT))), 4)

print(f'=== PRODUCTION MIN_FLOOR EVALUATION THRESHOLD: {eval_threshold} (Break-Even p*={p_star:.4f}) ===')
...
for w in range(N_WINDOWS):
    ...
    for i in range(w_start, w_end - 12):
        ...
        if norm_bull >= eval_threshold:
            ...
            continue  # Abstain under production MIN_FLOOR
...
print(f'\n======================================================================')
print(f'   PRODUCTION-ALIGNED (MIN_FLOOR={eval_threshold}) WALK-FORWARD RESULTS')
print(f'======================================================================')
