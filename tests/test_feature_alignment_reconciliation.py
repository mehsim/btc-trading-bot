...
def test_live_vs_backtest_feature_reconciliation():
    ...
    # 2. Live Streaming Window Simulation (bar by bar streaming into a rolling buffer of 80 bars)
    rolling_buffer = df_full.iloc[:80].copy()
    df_live_window = add_features(rolling_buffer)
    
    # Re-evaluate for candle 85 in live streaming mode
    rolling_buffer_85 = df_full.iloc[:85].copy()
    df_live_window_85 = add_features(rolling_buffer_85)

    # Compare common numeric feature columns for candle 85
    numeric_cols = df_backtest.select_dtypes(include=[np.number]).columns
    ignored_cols = ["timestamp", "target_trend", "target_price_change"]
    feature_cols = [c for c in numeric_cols if c not in ignored_cols and c in df_live_window_85.columns]

    backtest_row = df_backtest.iloc[84][feature_cols].astype(float).values
    live_row = df_live_window_85.iloc[84][feature_cols].astype(float).values
