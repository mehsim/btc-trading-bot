...
p_star = SL_MULT / (TP_MULT + SL_MULT)
cost_bps = 16.0
eval_threshold = round(min(0.52, max(MIN_EVAL_THRESHOLD_FLOOR, p_star + (cost_bps / 1e4) / (TP_MULT + SL_MULT))), 4)

print(f"=== FLIPPED DIRECTION (BULLISH <-> BEARISH) 15-WINDOW WALK-FORWARD BACKTEST ({eval_threshold}) ===")

# Load symbols individually so indexing is 100% correct per symbol
...
print("\n--- PER-WINDOW FLIPPED SIGNAL PERFORMANCE ---")
...
for w in range(N_WINDOWS):
    ...
    for i in range(w_start, w_end):
        ...
        # FLIP SIGNAL DIRECTION: If model says Bullish -> trade Bearish. If Bearish -> trade Bullish.
        ...
        # GUARD CHECK: Skip if symbol position is active until future exit timestamp
        if ts < active_until_ts.get(sym, 0):
            skipped += 1
            continue
            
        # Perform exit evaluation on THIS SYMBOL'S OWN candle series
        df_s = symbol_dfs[sym]
        s_idx = df_all.loc[i, "sym_idx"]
        
        if s_idx + 12 >= len(df_s):
            continue
...
print(f"\n===================================================================================")
print(f"   FLIPPED SIGNAL DIRECTION WALK-FORWARD SUMMARY")
print(f"===================================================================================")
